...
class RuntimePlanningEngineTests(unittest.TestCase):
    ...
    def test_six_lm_actions_are_swarm_coordinated_and_deconflicted(self) -> None:
        latitude = 37.4
        longitude = 127.9
        engine = RuleBasedPlanningEngine(
            latitude,
        ...
        target = self._target(latitude + 0.01, longitude)
        # Re-evaluate repeatedly while every LM is still travelling toward its
        # previous assignment.  This is the condition which formerly pinned
        # multiple vehicles to the same ARC radius after a later RHP refresh.
        for revision, elapsed_s in enumerate((1.0, 26.0, 51.0, 76.0), 1):
            ...
            self.assertTrue(result.planner["swarm_coordination"])
            self.assertEqual(set(range(1, 7)), set(result.route_updates))
            assigned_radii = {
                round(float(candidate["radius_m"]), 3)
                for candidate in result.route_candidates.values()
            }
            self.assertEqual(6, len(assigned_radii))
            endpoints = {
                (
