...
class RHPFEPFPWARCPlanner:
    ...
    def evaluate(
        ...
        *,
        ...
        radial_values = self._radial_values(belief, radii)
        shortlist_count = max(
            self.radial_shortlist_count,
            len(eligible) if self.swarm_coordination else 0,
        )
        shortlisted = sorted(
            radii,
    ...
    def _build_action(
        ...
        start_angle = (
            ...
            else math.tau * (vehicle_id - 1) / 6.0
        )
    ...
    def _arc_points(
        ...
        ratio = min(
            scan_distance_m / (2.0 * radius_m * subdivisions),
            1.0,
        )
        total_angle = 2.0 * subdivisions * math.asin(ratio)
        return tuple(
            self._polar_point(
                radius_m,
                start_angle + direction * total_angle * index / subdivisions,
            )
