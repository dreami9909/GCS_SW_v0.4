...
def main() -> int:
    ...
    if args.smoke_test:
        ...
        from PySide6.QtWidgets import QScrollArea, QStyle, QStyleOptionSpinBox
    ...
    if args.smoke_test:
        ...
        for aiming_spin in (
            window.details_view.azimuth_input,
            window.details_view.elevation_input,
        ):
            aiming_option = QStyleOptionSpinBox()
            aiming_spin.initStyleOption(aiming_option)
            aiming_up_button = aiming_spin.style().subControlRect(
                QStyle.ComplexControl.CC_SpinBox,
                aiming_option,
                QStyle.SubControl.SC_SpinBoxUp,
                aiming_spin,
            )
            aiming_down_button = aiming_spin.style().subControlRect(
                QStyle.ComplexControl.CC_SpinBox,
                aiming_option,
                QStyle.SubControl.SC_SpinBoxDown,
                aiming_spin,
            )
            assert aiming_up_button.width() >= 24
            initial_aiming_value = aiming_spin.value()
            QTest.mouseClick(
                aiming_spin,
                Qt.MouseButton.LeftButton,
                Qt.KeyboardModifier.NoModifier,
                aiming_up_button.center(),
            )
            assert aiming_spin.value() == initial_aiming_value + 1.0
            QTest.mouseClick(
                aiming_spin,
                Qt.MouseButton.LeftButton,
                Qt.KeyboardModifier.NoModifier,
                aiming_down_button.center(),
            )
            assert aiming_spin.value() == initial_aiming_value
